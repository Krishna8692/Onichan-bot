...
_ANIMATED_EMOJI = {
    ...
    "⌛": "5451732530048802485", "⏹": "6325507973896472524",
    "⚙": "5454415424319931791", "🌐": "5445284980978621387",
    "🎁": "5368324170671202286", "🎟": "5418010521309815154",
    "🎯": "5467583879948803288", "🎲": "5467583879948803288",
    "🏠": "5375296873982604963", "🏥": "5472146462362048818",
    "🏷": "5197269100878907942", "👁": "5188217332748527444",
    "💀": "5852812849780362931", "📜": "5197269100878907942",
    "📞": "5465169893580086142", "📡": "5424818078833715060",
    "📦": "5197269100878907942", "🔑": "5472308992514464048",
    "🔔": "5424818078833715060", "🔕": "6325636152900453913",
    "🗂": "5431376038628171216", "🛍": "5375296873982604963",
    "🛒": "5375296873982604963", "🤝": "5467406098367521267",
    # Colour circles & misc used in /conv and elsewhere
    "🟢": "5895458739703517004", "🔴": "5852753450382659113",
    "🔵": "5472308992514464048", "🟡": "5413704112220949842",
    "📍": "5431449001532594346", "💧": "5379600444098093058",
    "🧿": "5472308992514464048", "🟣": "5471952986970267163",
}
...
_MAGIC_THEMES = [
    ...
    {   # 6 — Ice / Snow / All Gray
        "gates": EID["search"],    "tools": EID["card"],
        "premium": EID["lock"],    "stats": EID["plan"],
        "help": EID["question"],   "admin": EID["plug"],     "channel": EID["link"],
        "magic": EID["back"],      "label": "🧊 Magic",
        "st_gates": "default", "st_tools": "default",
        "st_premium": "default", "st_stats": "default",
        "st_help": "default",  "st_admin": "default",  "st_channel": "default",
        "st_magic": "default",
    },
    {   # 7 — Sunset / Alternating Red-Green
        "gates": EID["live"],      "tools": EID["free"],
        "premium": EID["risky"],   "stats": EID["ticket"],
        "help": EID["error"],      "admin": EID["regenerate"],"channel": EID["back"],
        "magic": EID["stopped"],   "label": "🌅 Magic",
        "st_gates": "danger",  "st_tools": "success",
        "st_premium": "danger",  "st_stats": "success",
        "st_help": "danger",   "st_admin": "success",  "st_channel": "danger",
        "st_magic": "success",
    },
    {   # 8 — Ocean / Alternating Green-Purple
        "gates": EID["infinity"],  "tools": EID["link"],
        "premium": EID["users"],   "stats": EID["broadcast"],
        "help": EID["plan"],       "admin": EID["search"],   "channel": EID["plug"],
        "magic": EID["regenerate"],"label": "🌊 Magic",
        "st_gates": "success", "st_tools": "primary",
        "st_premium": "success", "st_stats": "primary",
        "st_help": "success",  "st_admin": "primary",  "st_channel": "success",
        "st_magic": "primary",
    },
    {   # 9 — Gold / Alternating Red-Gray
        "gates": EID["crown"],     "tools": EID["card"],
        "premium": EID["stats"],   "stats": EID["users"],
        "help": EID["ticket"],     "admin": EID["lock"],     "channel": EID["broadcast"],
        "magic": EID["welcome"],   "label": "🏆 Magic",
        "st_gates": "danger",  "st_tools": "default",
        "st_premium": "danger",  "st_stats": "default",
        "st_help": "danger",   "st_admin": "default",  "st_channel": "danger",
        "st_magic": "default",
    },
]
...
async def _bypass_3ds_dotbypasser(card: dict, checkout_url: str, pk: str = "",
                                   ...
                                   pi_id: str = "") -> dict:
    """Attempt 3DS bypass by re-confirming the PaymentIntent directly via Stripe's API.
    Works for custom-checkout PIs. For Checkout-Session PIs Stripe blocks the direct
    confirm and we return 3DS_REQUIRED gracefully.
    Returns a result dict with 'status' (CHARGED/DECLINED/LIVE/3DS_REQUIRED/ERROR)."""
    import asyncio as _asyncio

    if not pk or not pi_id or not pi_client_secret:
        missing = [k for k, v in [("pk", pk), ("pi_id", pi_id), ("pi_client_secret", pi_client_secret)] if not v]
        print(f"[bypass] skipped — missing: {missing}", flush=True)
        return {"status": "3DS_REQUIRED", "response": "3DS 🔐"}
    ...
    def _do_request():
        ...
        # Call Stripe directly — never proxy
        try:
            resp = _r.post(url, data=data, headers=headers, timeout=30)
    ...
    if err_obj:
        # error may be a plain string from some proxy/API layers
        if isinstance(err_obj, str):
            ...
            err_msg = err_obj.get("message", "Declined")
        tag = decline or code
        msg = err_msg[:80]
        ...
        # Checkout-session PIs can't be confirmed via the PI endpoint
        _msg_low = msg.lower()
        if ("cannot perform this action" in _msg_low or "unexpected_state" in code):
            return {"status": "3DS_REQUIRED", "response": "3DS 🔐"}
        # Other Stripe request errors — not a card result
        if not decline:
            return {"status": "3DS_REQUIRED", "response": "3DS 🔐"}
        return {"status": "DECLINED", "response": f"[{tag}] {msg}" if tag else msg}
    ...
    if pi_status == "requires_payment_method":
        ...
        lpe_msg = lpe.get("message", "Declined")[:80]
        return {"status": "DECLINED", "response": f"[{dc}] {lpe_msg}" if dc else lpe_msg}
    if pi_status == "processing":
        return {"status": "LIVE", "response": "Payment processing"}
    # requires_action / requires_confirmation / unknown → still 3DS
    return {"status": "3DS_REQUIRED", "response": "3DS 🔐"}
