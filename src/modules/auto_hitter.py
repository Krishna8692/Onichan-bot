            ...
            for attempt in range(3):
                try:
                    ...
                    if "error" not in init_data:
                        ...
                        # Extract Stripe Connect account ID (acct_xxx) if this is a Connect platform checkout
                ...
                if use_direct_confirm:
                    if checkout_mode == "setup":
                        conf_body = (
                            ...
                            f"&payment_method_data[billing_details][address][line1]={quote(line1)}"
                            f"&payment_method_data[billing_details][address][city]={quote(city)}"
...
def format_charge_result(result: dict, info: dict, card: dict = None) -> str:
    ...
    card_str = full_card(card) if card else "N/A"
    merchant = html.escape(str(info.get('merchant') or 'Unknown'))
